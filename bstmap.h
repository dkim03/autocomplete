...
private:
  ...
  // function to convert a sorted vector to a BST
  Node* vectorToBST(const vector<value_type> &vec, int start, int end);

  // function to convert a BST to a sorted vector
  void bstToVector(vector<value_type> &vec, const Node* root) const;

  // misc helper functions
  void copyHelper(vector<value_type> &vec, const Node* root) const;
  void clearHelper(Node* &root) const;
  bool containsHelper(const key_type &key, const Node* root) const;
  bool eraseHelper(const key_type &key, Node* root);
  void getAllHelper(const key_type &key, vector<value_type>& vec, const Node* root) const;
  mapped_type& retrieve(const key_type &key, Node* root) const;
  int heightHelper(Node* root) const;

  // helper functions for recursive traversal
  void inorderHelper(const Node* root, void visit(const value_type &item)) const;
  void preorderHelper(const Node* root, void visit(const value_type &item)) const;
  void postorderHelper(const Node* root, void visit(const value_type &item)) const;
  ...
  // helper function to sort a vector in ascending order using std::sort
  bool static sortStringAscending(BSTMap::value_type &a, BSTMap::value_type &b);
};
